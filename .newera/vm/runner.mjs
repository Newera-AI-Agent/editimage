...
async function main() {
  ...
  for (step = 1; step <= MAX_STEPS; step++) {
    ...
    try {
      ...
      else result = { ok: false, output: "unknown tool: " + toolName + " — available: shell, list_files, list_skills, read_skill, read_file, write_file, append_file, edit_file, delete_file, generate_image, spawn_agents, request_deploy, finish" };
    } catch (err) {
      result = { ok: false, output: "tool error: " + (err.message || String(err)) };
    }
